...
export class UniversalContextGenerator {
  ...
  async generateUniversalContext(
    ...
    try {
      ...
      for (const assistantType of assistantTypes) {
        const output = this.buildOptimizedOutput(
          optimizedFiles,
          stats,
          options,
          timestamp,
      ...
      throw new Error(
        `Failed to generate universal context: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async collectProjectFiles(): Promise<FileInfo[]> {
    try {
      ...
      Logger.error(
        'Failed to collect project files:',
    ...
    return {
      ...
      projectDetection: {
        ...
        customConfig: {
          ...
          hasPagesRouter:
            projectStructure.type === 'pages-router' || projectStructure.type === 'mixed',
        },
  ...
  private buildWindsurfOptimizedOutput(
    ...
    if (stats.projectDetection?.detectedLibraries) {
      ...
      if (libs.testing.length > 0) {
        output += `| **Testing** | ${libs.testing.join(', ')} |\n`;
      }
      output += '\n';
    }
  ...
  private async saveUniversalOutput(
    output: string,
    options: UniversalContextOptions
  ): Promise<void> {
    const optimizeFor = options.optimizeFor || ['universal'];
    ...
    const savePromises = optimizeFor.map(async format => {
      ...
      try {
        ...
        Logger.error(
          `Error saving universal context: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    });
  ...
  private generateComponentRelationships(
    files: FileInfo[]
  ): Array<{ component: string; relationships: string[] }> {
  ...
  private extractFeatureFromPath(filePath: string): string {
    ...
    if (filePath.includes('/styles/') || filePath.includes('.css')) {
      return 'Styling';
    }
  ...
  private detectNamingConvention(files: FileInfo[]): string {
    const kebabCase = files.filter(f =>
      /^[a-z][a-z0-9-]*$/.test(path.basename(f.path, path.extname(f.path)))
    ).length;
    const camelCase = files.filter(f =>
      /^[a-z][a-zA-Z0-9]*$/.test(path.basename(f.path, path.extname(f.path)))
    ).length;
    const pascalCase = files.filter(f =>
      /^[A-Z][a-zA-Z0-9]*$/.test(path.basename(f.path, path.extname(f.path)))
    ).length;

    if (kebabCase > camelCase && kebabCase > pascalCase) {
      return 'kebab-case';
    }
  ...
  private detectComponentPattern(files: FileInfo[]): string {
    ...
    if (hasClientComponents && hasServerComponents) {
      return 'Mixed (Client + Server Components)';
    }
  ...
  private summarizeFileContent(content: string, filePath: string): string {
    ...
    if (['.ts', '.tsx', '.js', '.jsx'].includes(extension)) {
      ...
      const exports =
        content.match(/(export\s+(default\s+)?(function|const|class|interface|type).*)/g) || [];
      if (exports.length > 0) {
        summary.push('// KEY EXPORTS');
        summary.push(
          ...exports.map(exp => exp.split('{')[0] + (exp.includes('{') ? ' { ... }' : ''))
        );
        summary.push('');
      }
  ...
  private mapProjectStructureType(
