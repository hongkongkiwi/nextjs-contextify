    ...
    const steps = [
      {
        ...
        description:
          this.selectedFilesCount > 0
            ? `${this.selectedFilesCount} files selected`
            : 'No files selected',
        command: {
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const generateQuickContext = vscode.commands.registerCommand(
  ...
  const generateWithPrompts = vscode.commands.registerCommand(
  ...
  // New UI commands
  const openContextifyUI = vscode.commands.registerCommand(
  ...
  const refreshTreeView = vscode.commands.registerCommand(
  ...
  const selectAllFiles = vscode.commands.registerCommand(
  ...
  const deselectAllFiles = vscode.commands.registerCommand(
  ...
  const onTreeItemClick = vscode.commands.registerCommand(
    'nextjsContextifyExplorer.onItemClick',
    (item: FileTreeItem) => {
  ...
  // Focus commands
  const focusFileSelection = vscode.commands.registerCommand(
    'nextjsContextifyExplorer.focus',
    () => {
...
// Webview UI Function
async function showContextifyUI(
    ...
    await vscode.window.withProgress(
      ...
      async progress => {
        ...
        const outputPath = path.join(
          rootPath,
          `nextjs-contextify-${options.format}.${getFileExtension(options.format)}`
        );
  ...
  const format = await vscode.window.showQuickPick(
    [
      { label: 'XML Format', detail: 'Structured XML tags for better LLM parsing', value: 'xml' },
      {
        ...
        detail: 'Traditional markdown with code blocks',
        value: 'markdown',
      },
    ...
    [
      {
        ...
        detail: 'Optimized for Claude with 200k+ context',
        value: 'claude',
      },
...
async function showPromptWizard(context: vscode.ExtensionContext): Promise<void> {
  ...
  const promptType = await vscode.window.showQuickPick(
    [
      {
        label: '🐛 Bug Fix Analysis',
        detail: 'Generate context for debugging specific issues',
        value: 'bug-fix',
      },
      {
        label: '🔄 Refactoring Help',
        detail: 'Context for large-scale code refactoring',
        value: 'refactor',
      },
      {
        label: '📚 Documentation Generation',
        detail: 'Create comprehensive documentation',
        value: 'docs',
      },
      { label: '🔍 Code Review', detail: 'Prepare for thorough code review', value: 'review' },
      {
        label: '🚀 Feature Development',
        detail: 'Context for new feature implementation',
        value: 'feature',
      },
      {
        label: '⚡ Performance Optimization',
        detail: 'Analyze for performance improvements',
        value: 'performance',
      },
      {
        label: '🏗️ Architecture Analysis',
        detail: 'Deep dive into project architecture',
        value: 'architecture',
      },
    ],
    {
  ...
  await generateCodebaseContextWithPrompts(
    workspaceFolders[0].uri.fsPath,
    options,
    promptType.value
  );
...
function buildEnhancedOutputWithUI(
  ...
  if (
    ...
    EXTENDED_PROMPT_LIBRARY[options.selectedPrompt as keyof typeof EXTENDED_PROMPT_LIBRARY]
  ) {
    const prompt =
      EXTENDED_PROMPT_LIBRARY[options.selectedPrompt as keyof typeof EXTENDED_PROMPT_LIBRARY];
    output = `# ${prompt.title}
